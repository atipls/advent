...
class CubeSet:
    ...
    def possible(self, red: int, green: int, blue: int):
        return (
            self.get_count("red") <= red
            and self.get_count("green") <= green
            and self.get_count("blue") <= blue
        )
...
class Game:
    ...
    def smallest(self):
        return (self.smallest_red(), self.smallest_green(), self.smallest_blue())
...
games = [parse_game_data(line) for line in data]

fewest_cubes_per_game = [game.smallest() for game in games]

for index, game in enumerate(fewest_cubes_per_game):
    print(games[index], game)

powers = [red * green * blue for red, green, blue in fewest_cubes_per_game]
