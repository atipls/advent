...
class Mapping:
    ...
    def to_destination_category(self, src: int) -> int | None:
...
class Parcel:
    ...
    def parse():
        ...
        total_seeds = sum(length for _, length in seed_ranges)
        print("A total of {} seeds".format(total_seeds))
...
parcel = Parcel.parse()
current_minimum = [sys.maxsize] * len(parcel.seeds)

seeds_checked = 0
start_time = time.time()

print("Warning! This will take over 4 hours to run!")
for index, (seed_start, length) in enumerate(parcel.seeds):
    print("Checking seeds {} to {} ({})".format(seed_start, seed_start + length, length))
    for seed_index in range(length):
        location = parcel.map_seed_to_location(seed_start + seed_index)
        seeds_checked += 1
        elapsed = time.time() - start_time
        if elapsed > 10:
            print("Progress: {:.2f}%".format(seeds_checked / parcel.total_seeds * 100))
            start_time = time.time()
        if location < current_minimum[index]:
            current_minimum[index] = location

    print("Minimum: {}".format(current_minimum[index]))
        
print("Candidates: {}".format(current_minimum))
print("The lowest seed is {}".format(min(current_minimum)))